...
def check_nodename(nodename, ip):
...
check_nodename(args.nodename, args.ip)
...
process = subprocess.Popen("wwsh node new "+args.nodename+" --netdev="+args.interface+" --ipaddr="+args.ip+" --hwaddr="+args.hwaddr.split(), stdout=subprocess.PIPE)
...
process = subprocess.Popen("wwsh file import "+netconfig+args.nodename+'-'+args.dataint+" --path=/etc/sysconfig/network-scripts/ifcfg-"+args.dataint.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
print(output)
if error != "":
    print(error)
...
process = subprocess.Popen("wwsh provision set "+args.nodename+"  --vnfs="+vnfs+" --bootstrap="+bootstrap+" --files=dynamic_hosts,passwd,group,shadow,munge.key,network,"+args.nodename+'-'+args.dataint.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
print(output)
if error != "":
    print(error)
...
print("ssh "+args.nodename+" init 6")
